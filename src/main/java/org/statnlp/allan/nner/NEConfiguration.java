 ...
 *
 * There is no root node. and this one is not nested.
 * Currently, we try one with shift(t), shift one entity only
 *
 * @author Allan Jie
 */
public class NEConfiguration {
  ...
  /**
   * @param k Word index (indexed from 0)
   */
   ...
   *
   * @return Sentence index or {@link NEConfig#NONEXIST} if stack doesn't
  ...
  /**
   * Returns a string that concatenates all elements on the stack and buffer, as well as named entities
   * @return
   */
