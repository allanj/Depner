...
public class NEDependencyTree {
  ...
  public NEDependencyTree() {
    n = 0;
    head = new ArrayList<>();
    head.add(NEConfig.NONEXIST);
    label = new ArrayList<>();
    label.add(NEConfig.UNKNOWN);
  }

  public NEDependencyTree(NEDependencyTree tree) {
    n = tree.n;
  ...
  public boolean equal(NEDependencyTree t) {
    if (t.n != n)
      return false;
    for (int i = 1; i <= n; ++i) {
